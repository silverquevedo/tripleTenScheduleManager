      ...
      <div className="relative flex-shrink-0" ref={employeeDropdownRef}>
        ...
        >
          {selectedMembers.length === 0 ? (
            ...
            <>
              <span className="w-2.5 h-2.5 rounded-full flex-shrink-0 border" style={{ backgroundColor: getBadgeColor(members.find((m) => m.displayName === selectedMembers[0])?.color ?? '').bg, borderColor: getBadgeColor(members.find((m) => m.displayName === selectedMembers[0])?.color ?? '').border }} />
            ...
            <>
              <div className="flex -space-x-1">
                {selectedMembers.slice(0, 3).map((name) => (
                  <span key={name} className="w-2.5 h-2.5 rounded-full flex-shrink-0 border" style={{ backgroundColor: getBadgeColor(members.find((m) => m.displayName === name)?.color ?? '').bg, borderColor: getBadgeColor(members.find((m) => m.displayName === name)?.color ?? '').border }} />
                ))}
              </div>
          ...
          <div className="absolute top-full left-0 mt-1 bg-white border border-[#e5e5e3] rounded-xl shadow-lg z-20 min-w-[170px] py-1">
            ...
            {members.map((m) => (
              <button key={m.id} onClick={() => toggleMember(m.displayName)} className="w-full flex items-center gap-2 px-3 py-1.5 hover:bg-gray-50 text-xs text-left">
                <span className="w-3 h-3 rounded-full flex-shrink-0 border" style={{ backgroundColor: getBadgeColor(m.color).bg, borderColor: getBadgeColor(m.color).border }} />
